...
class TestCMScraper(unittest.TestCase):
    ...
    @patch('requests.get')  # Mock requests.get for enhancement price fetching
    def test_fetch_enhancement_prices(self, mock_get):
        # Simulate a valid API response for enhancement prices
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            'history': {0: 5000000, 1: 5500000, 2: 6000000, 3: 6500000, 4: 7000000}
        }
        mock_get.return_value = mock_response

        # Simulate an accessory to test with
        accessory = {
            'name': 'Test Accessory',
            'id': 12345,
            'currentStock': 100,
        ...
        # Call the function to fetch enhancement prices
        prices = cmscraper.fetch_enhancement_prices(accessory['id'])

        # Check if the prices are returned correctly
        self.assertEqual(prices, [5000000, 5500000, 6000000, 6500000, 7000000])
